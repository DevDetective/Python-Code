...
from app.utils import tf_idf_cosine_similarity as tf_idf
...
from app.models import AI_Recruitment_model#import db
...
def extract_skills(input_text, skill_list):
    try:
        ...
        print ('''exception in scoring_helpers.extract_skills fun ''', str(e))
        return []
...
def screening_main(all_urls=[], job_description='', all_resumes={}, tree_sheet_path=''):
    """
    ...
    #     all_urls = db.return_all_docs_url(db.mycol)
    ...
    all_resumes_list = []
    for key in all_resumes.keys():
        all_resumes_list.append(all_resumes[key]) 
    all_resumes_text = all_resumes_list
    # all_resumes_text = read_all_resumes(all_resumes)

    try:
        for profile in all_urls:

            
            my_profile = list(AI_Recruitment_model.objects.filter(url=profile).values())#db.find_document_on_url(db.mycol,profile)
            # my_profile = data[profile]

            my_name = json.loads(my_profile[0]['linkedin_data'])['name']
            ...
            if 'experience' in json.loads(my_profile[0]['linkedin_data']).keys():
                experience_list = json.loads(my_profile[0]['linkedin_data'])['experience']
            ...
            if 'skills' in json.loads(my_profile[0]['linkedin_data']).keys():
                for skill in json.loads(my_profile[0]['linkedin_data'])['skills']:
                    my_skills += ' ' + skill.lower()
                    linkedin_skill_list.append(skill.lower())
            if 'about' in json.loads(my_profile[0]['linkedin_data']).keys():
                my_about += ' ' + json.loads(my_profile[0]['linkedin_data'])['about'].lower()
